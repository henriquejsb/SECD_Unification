...
compile(App e1 e2) sym = (compile e1 sym) ++ [LDVF (compile e2 sym ++ [RTN])] ++ [AP]
...
execute(s,e,LD i:c,d,m) = let v = e!!i
                          in 
                          if not (isVA v)
                            then
                            (v:s,e,c,d,m)
...
execute(s,e,(LDVF c'):c,d,m) = (VA addr:s,e,c,d,Map.insert addr (c',e) m) 
                              where addr = next m
